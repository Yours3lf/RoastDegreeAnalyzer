...
void compressAndSend(uint8_t *data, uint32_t length, uint32_t type, uint32_t width, uint32_t height)
{
	...
	if (type == 1)
	{
		...
		ic.compressImage<uint8_t>(resBuf, data, length, 8, 4, width, height, 8);
	...
	else if (type == 2)
	{
		...
		std::vector<uint8_t> resBuf;
		ic.compressImage<uint16_t>(resBuf, rChannel.data(), rChannel.size() * sizeof(uint16_t), 10, 1, width >> 1, height >> 1, 8);
		m->buf.resize(sizeof(uint32_t) + resBuf.size());
		*(uint32_t *)(m->buf.data()) = type;
		memcpy(m->buf.data() + sizeof(uint32_t), resBuf.data(), resBuf.size());
	}
...
void compressionThreadFunc()
{
	...
	{
		uint32_t width, height, type, length;
		uint8_t *data;
		{
		...
		compressAndSend(data, length, type, width, height);
...
int main(int argc, char **args)
{
	...
	controlList.set(controls::ExposureValue, 1.0f);
	controlList.set(controls::ExposureTimeMode, controls::ExposureTimeModeManual); // microseconds
	controlList.set(controls::ExposureTime, exposureTime); // microseconds
	
	controlList.set(controls::AnalogueGainMode, controls::AnalogueGainModeManual); // float, must be >=1.0
